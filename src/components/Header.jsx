...
function Header({toggleBar, setToggleBar}) {
  ...
  return (
   <div className=' w-full h-auto relative bg-transparent ' >
    ...
    <div className=' bg-zinc-100 p-4 shadow-sm flex justify-between items-center z-30 '>
      <GiHamburgerMenu onClick={toggleMenuEvent} color='#164e63' size={30} className=' cursor-pointer ' />
      <div className='flex justify-between items-center'>
        <h3 className=' mr-5 text-cyan-900 '>Hello, {loginAdmin?.firstname} </h3>
        <div className=' relative w-12 h-12 flex justify-center items-center ' >
            <IoNotificationsSharp size={40} color='#164e63' className=' cursor-pointer ' onClick={()=>setNotificationToggle((state)=>{return !state;})} />
            {
    ...
    {/* NOTIFICATION BOX */}
  {
      isNotificationToggle && (
        <div className=' bg-white w-96 h-96 max-h-96  overflow-auto absolute rounded-br-md rounded-bl-md shadow-lg top-20 right-5 p-3 z-30 '>
          {/* <div className=' fixed bg-green-300 w-full z-20 '>
wadw
          </div> */}
        {
