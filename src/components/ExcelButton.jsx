...
function ExcelButton({ users, title,authorizedPerson, time }) {
  let fields;
  ...
  const printExcel = async () => {
    ...
    const dataStartingRow = 4;
    const authorizedPersonRow = 2; // Add this line

    // Add authorized person to the worksheet
    const authorizedPersonCell = worksheet.getRow(authorizedPersonRow).getCell(1);
    authorizedPersonCell.value = `Generated by: ${authorizedPerson} (${time})`;
    authorizedPersonCell.style = {
      font: { size: 12,},
      ...
      top: { style: 'thin' },
      bottom: { style: 'thin', size: 50 }, // Add vertical padding of 10
    };

    // Merge cells for the authorized person
    const numColumns = fields.length; // Assuming each field corresponds to a column
    worksheet.mergeCells(`A${authorizedPersonRow}:${String.fromCharCode(64 + numColumns)}${authorizedPersonRow}`);
