		...
		<div className='py-3 flex justify-center '>
			...
			</button>
			{
				pageNumber.map((page, index) => (
					<button key={index} className={`${currentPage == page ? 'bg-blue-700 text-white' : ''} py-2 px-4 text-blue-900 border-gray-500 cursor-pointer flex items-center hover:bg-blue-600 hover:text-white`} onClick={() => { setCurrentPage(page) }} >
						{page}
					</button>
				))
			}
