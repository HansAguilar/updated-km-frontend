...
function Profile() {
  const {adminLoading, loginAdmin } = useSelector((state) => state.admin);
  const dispatch = useDispatch();
  const [adminInfo, setAdminInfo] = useState({
    firstname: loginAdmin.firstname,
    middlename: loginAdmin.middlename ? loginAdmin.middlename : "",
    lastname: loginAdmin.lastname,
    birthday: loginAdmin.birthday,
    contactNumber: loginAdmin.contactNumber,
    address: loginAdmin.address,
  ...
  const submitButton = (e) => {
    ...
    if(adminInfo.password || adminInfo.confirmPassword){
      if (!haveGhostAndEmojisChar.test(adminInfo.password) || !haveGhostAndEmojisChar.test(adminInfo.confirmPassword)) return toastHandler("error", "Password must be at least 8 characters long");

      if (adminInfo.password.replace(/\s+/g, '') !== adminInfo.confirmPassword.replace(/\s+/g, '')) return toastHandler("error", "Passwords do not match");
    }

    const data = {
      ...adminInfo,
      profile: !profile ? adminInfo.profile : profile
    }
    dispatch(updateAdminLogin(loginAdmin.adminId, data))
  }

  return (
    <>
      {
        adminLoading ? <div className=' w-full h-screen flex justify-center items-center '>
              ...
              <div className='flex flex-col items-center w-[28rem] gap-8'>
                ...
                <div className='bg-white w-full h-72 flex items-center justify-center rounded'>
                  {
                    !loginAdmin.profile
                      ...
                      : <img src={loginAdmin.profile} className='h-52 w-52 aspect-auto rounded-full border-2' alt='admin' />
                  }
                </div>
                <div className='bg-white divide-y flex items-center flex-col justify-between w-full gap-4 p-4 rounded flex-1'>
                  ...
                  <div className=' w-full flex justify-between items-center p-2 border-gray-300 '>
                    ...
                    <p className='capitalize text-slate-600'>{`${adminInfo.firstname} ${!adminInfo.middlename ? "" : adminInfo.middlename} ${adminInfo.lastname}`}</p>
                  </div>
                  {/*//~ NAME */}
                  ...
                  <div className=' w-full flex justify-between p-2 border-gray-300 '>
                    ...
                    <p className='capitalize text-slate-600'>{loginAdmin.gender}</p>
                  </div>
                  {/*//~ GENDER */}
                  ...
                  <div className=' w-full flex justify-between p-2 border-gray-300 '>
                    ...
                    <p className='capitalize text-slate-600'>{moment(adminInfo.birthday).format("MMM DD, YYYY")}</p>
                  </div>
                  {/*//~ BIRTHDAY */}
                  ...
                  <div className=' w-full flex justify-between p-2 border-gray-300 '>
                    ...
                    <p className='capitalize text-slate-600'>{adminInfo.contactNumber}</p>
                  </div>
                  {/*//~ PHONE */}
                  ...
                  <div className=' w-full flex justify-between p-2 border-gray-300 '>
                    ...
                    <p className='capitalize text-slate-600'>{adminInfo.address}</p>
                  </div>
                  {/*//~ ADDRESS */}
                  ...
                  <div className=' w-full flex justify-between p-2 mb-4 border-gray-300 '>
                    ...
                    <p className='capitalize text-slate-600'>{loginAdmin.role}</p>
                  </div>
                  {/*//~ ROLE */}
                </div>
              </div>
